...
class Checker:
  ...
  def draw(self):
    ...
    self.output = np.tile(white[0: self.tile_size * 2, 0 : self.tile_size * 2], (self.resolution/(self.tile_size*2), self.resolution/(self.tile_size*2)))
    pyp.xticks([])
    pyp.yticks([])
    pyp.imshow(self.output.copy(), cmap='gray')
...
class Circle:
  ...
  def show(self):
    plt.xticks([])
    plt.yticks([])
    plt.imshow(self.output.copy(), cmap='gray')
    plt.show()
